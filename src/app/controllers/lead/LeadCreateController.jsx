...
const LeadCreateController = () => {
    ...
    const validate = () => {
        if (inputs.lead_id == "") {
            handleValues('set', 'err', _lang('lead_id_required'))
            return 0
        }
        if (inputs.person_name == "") {
            handleValues('set', 'err', _lang('person_name_required'))
            return 0
        }
        if (inputs.contact_no == "") {
            handleValues('set', 'err', _lang('contact_no_required'))
            return 0
        }
        ...
        if (!inputs.type || inputs.type == "") {
            handleValues('set', 'err', _lang('lead_type_required'))
            return 0
        }
        if (!inputs.status || inputs.status == "") {
            handleValues('set', 'err', _lang('lead_status_required'))
            return 0
        }
    ...
    return (
        <>
            <LeadCreate fetchUserFun={fetchUserFun} assignToDropDown={assignToDropDown} onSubmit={onSubmit} handleValues={handleValues} loading={loading} />
        </>
    )
}
export default LeadCreateController